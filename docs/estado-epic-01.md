...
| T06 · "un PR con vulnerabilidad crítica queda bloqueado"                     | Parcial      | El job `deps` ya ha corrido en verde sobre el árbol limpio (run 34258293656). Lo que sigue sin probarse es que **bloquee** ante una vulnerabilidad crítica real          |
| T06 · "un PR que rompe las fronteras de arquitectura falla"                  | **Cumplido** | Probado con la violación real: un `import` de `@coord/db` desde `packages/core` da `error core-no-sale` y exit 1. El job `arch` corre en CI y pasa sobre el árbol limpio |
| T06 · "un PR limpio tarda menos de 10 minutos"                               | **Cumplido** | Medido, no estimado: run 34258293656, 9 jobs en verde en **2.0 minutos** de reloj                                                                                        |
...
| DoD del epic · "el Verifier ha emitido informe en contexto limpio"           | **Cumplido** | Seis verificaciones en contexto aislado (una por tarea), sin ver el razonamiento de quien codeó: 46 hallazgos, 12 graves, los 12 reparados. Ver §6                       |
...
| **Saldada.** Los workflows ya se han ejecutado: el primer push tumbó el CI porque `aquasecurity/trivy-action@0.36.0` no resolvía (el tag real lleva `v`), y el agregador `ci-ok` propagó el fallo como debía. Corregido, y verificadas contra la API de GitHub las cinco referencias de actions del pipeline. | Queda como aviso: la constitución obliga a comprobar que una dependencia de npm existe (CLAUDE.md §2.5), pero no decía nada de las **actions**, y era el mismo riesgo. |
