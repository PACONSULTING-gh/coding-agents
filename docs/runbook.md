...
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **RESUELTO el 11 de septiembre de 2026 (issue #48).** `parse/*` tenía **cero tests propios**: se ejercitaba de refilón desde `ingest`, 40 tests para 291 mutantes. Escritos tests puros que miran el árbol de tree-sitter, `parse/python.ts` sube de **49,83 a 86,60**, y `ecmascript.ts` 77,23, `typescript.ts` 85,71, `javascript.ts` 86,67. Los cuatro entran al gate. De paso destaparon un fallo real que ningún recuento podía ver: la gramática de JavaScript cuelga el identificador directamente de `class_heritage` y la de TypeScript lo envuelve en `extends_clause`, así que **ninguna herencia de un fichero `.js`/`.jsx` llegaba al grafo**. | Hecho.                                        |
