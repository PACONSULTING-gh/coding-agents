...
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **Mutation testing de `packages/graph`.** Único paquete que sigue fuera de la lista `mutate`. Tiene su propio soporte de test (13 ficheros) y todavía levanta un contenedor por fichero, así que bajo Stryker vuelve a ser uno por mutante y su puntuación la decidirían los timeouts. `packages/db` y `packages/queue` ya no: adoptaron el ADR 0007 y sus dos módulos están en el gate (`client.ts` 68,97 %, `pg-boss-queue.ts` 77,54 %). | Adoptar el ADR 0007 en el soporte de test de `packages/graph`. El patrón ya está probado en dos paquetes. |
