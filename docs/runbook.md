...
|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **Mutation testing de `packages/queue/src/pg-boss-queue.ts`.** Fuera de la lista `mutate`. Ya no por medir mal —eso lo arregló el ADR 0007, y `packages/db/src/client.ts` volvió a la lista con **68,97 %** y 94 mutantes muertos donde antes había cero— sino porque su cifra REAL es **42,86 %**: 72 muertos, **80 supervivientes** y 56 mutantes sin cobertura. Meterlo por debajo del `break=60` dejaría el workflow semanal en rojo permanente, y un gate siempre rojo se acaba ignorando igual que uno siempre verde. | Matar los 80 supervivientes. Son comportamiento real que hoy nadie comprueba: el mapeo de opciones y el enrutado a la cola de fallidos (issue #26). |
