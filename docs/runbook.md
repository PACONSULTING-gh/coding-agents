...
| **Mutation testing de `packages/graph`.** No está en la lista `mutate` de `stryker.config.json` pese a contener los módulos más críticos del epic 02 (las dos guardas de la CTE, los claims, el motor incremental, la resolución de especificadores). Medido con `stryker run --mutate packages/graph/src/parse/python.ts`: 291 mutantes, **230 estáticos (79 %)** —el paquete exporta objetos de nivel superior, así que Stryker re-ejecuta la suite entera por mutante—, y como cada ejecución levanta un Postgres con testcontainers el resultado fue **0 muertos, 229 timeouts, puntuación 78,69 que supera el `break=60` sin que ningún test haya matado nada**. Un umbral superado por timeouts es peor que no medir.                                                                                                                                                                                                                                                                                       | **ADR 0007** (propuesta), el mismo desbloqueo que para `packages/db` y `packages/queue`: que los tests reciban la URL de un Postgres ya en marcha en vez de levantarlo. Sigue siendo decisión humana.                                                                                                                  |
| **Mutation testing de `packages/db/src/client.ts` y `packages/queue/src/pg-boss-queue.ts`.** Salieron de la lista `mutate` el 9 de septiembre de 2026 por la misma razón que `packages/graph`: sus tests levantan un Postgres con testcontainers, así que la puntuación la decidían los timeouts y no los tests. Medido sobre el mismo commit cambiando solo `--concurrency`: `client.ts` daba **90,34 con 0 mutantes muertos, 131 timeouts y 0 supervivientes** con la concurrencia por defecto, y **77,93 con 55 muertos y 18 supervivientes reales** con `--concurrency 8`; `pg-boss-queue.ts` pasaba de **76,47 (2 muertos, 180 timeouts)** a **43,70 (88 muertos, 78 supervivientes)**. El gate estaba en verde escondiendo 96 mutantes vivos. Esos supervivientes son deuda real y siguen ahí (issue #26). `envelope.ts` sufre lo mismo en menor grado (100,00 → 93,48 → 86,96 según la concurrencia) y se queda en la lista con `concurrency` fijada a 4, que hace la cifra comparable pero no la arregla. | **ADR 0007** (propuesta): provisionar Postgres fuera del proceso de test. Se probó la vía barata —memoizar el contenedor con `isolate: false`— y NO sirve: bajo Stryker sigue saliendo un contenedor por mutante (medido, `1 → 31` en dos minutos). Hasta que se acepte el ADR, medirlos aquí es peor que no medirlos. |
